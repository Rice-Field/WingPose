...
def genheatmap(sigma, point, width, height):
    ...
    hm = pow(np.e, (-((xmap-x0)**2 + (ymap-y0)**2)/(2*sigma*sigma))) / (sqrt(2*np.pi)*sigma)
...
hm = np.zeros((193, nimg_h, nimg_w, 8), dtype=np.float32)
